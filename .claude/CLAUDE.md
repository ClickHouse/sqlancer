...
## Nightly false-positive fixes, 2026-09-01 (`docs/plans/2026-09-01-001-fix-nightly-oracle-false-positives.md`)

The `NightlySQLancer` job (ClickHouse/ClickHouse, `ci/jobs/sqlancer_job.sh`) went red on 2026-08-25, 08-28, 08-31 and 09-01. Ten of the eleven 09-01 findings are the still-unfiled `NOT (NOT key)` value-position bug documented under the 2026-08-15 batch above, reported through `CountOptimization`, `PartitionMirror`, `SubqueryMaterialize`, `TLPAggregate`, `TLPDistinct`, `TLPGroupBy`, `ViewEquivalence`, `ReadInOrderToggle`, `FinalMerge` and `TLPSetOp`; triage by `(((NOT ((NOT (` in the failing predicate and do not gate it. The eleventh was a fork defect, fixed here together with a reproducer-content defect.

**Merging engines are excluded from every read-twice oracle.** `ClickHouseTable.isStableForRepeatedReads()` is false for `ReplacingMergeTree`, `SummingMergeTree`, `AggregatingMergeTree`, `CollapsingMergeTree` and `VersionedCollapsingMergeTree` (the same set as `supportsFinal()`), and both oracle-facing table pickers apply it: `ClickHouseSchema.getRandomTableNonEmptyTables()` (26 ClickHouse-specific oracles) and `ClickHouseExpressionGenerator.setTablesAndColumns` (the shared `TLPWhere` and `NoREC` oracles, which pick their tables through `TestOracleUtils` and bypass the schema override). The 09-01 `TLPWhere` finding (`216 vs 204` rows on `SELECT * FROM t1, t0`) was a background merge collapsing two `AggregatingMergeTree` parts between the two reads; the TLP partition was sound, the relation underneath it was not. Mandatory `FINAL` was rejected as the fix because `FINAL` does not make these engines stable either: non-summed `SummingMergeTree` columns, plain `AggregatingMergeTree` columns, tied-`ver` `ReplacingMergeTree` rows and `CollapsingMergeTree` sign sequences are all ANY-like, so a merge between two `FINAL` reads still flips the visible multiset (that is exactly what commit `3a66cc5c` observed in `TLPSetOp`). Consequences: `ClickHouseTLPBase` no longer emits `FINAL` and `TLPSetOp` lost its now-dead `FINAL` guard; persistent `v<n>` views are only created over stable tables (`ClickHouseViewGenerator`), so a view is stable by construction; an oracle that *wants* a merging engine must opt in explicitly via `getDatabaseTables()` plus `supportsFinal()` (`FinalMerge`, `ClickHouseOptimizingOracle`) or build a private fixture (`ReplacingDedup`, `CoalescingFinal`, `TupleFinalAggregation`, `EngineEquivalence`). Generators (`INSERT`, `ALTER`, mutations, `OPTIMIZE`) and single-statement oracles such as `AggregateStateRoundtrip` go through `getDatabaseTables*()` and still see those engines; a single read cannot race a merge, two reads can.

**Every oracle read must reach the reproducer.** `ComparatorHelper.getResultSetFirstColumnAsString` writes the SQL only to the rolling `databaseN-cur.log`; the saved `databaseN.log` is built from `StateToReproduce.statements`, which only `state.getState().logStatement(sql)` appends to. `CountOptimization`'s 09-01 `database12.log` (30 MB) contained none of its six statements, and its `AssertionError` carried only the predicate and two counts. Fixed by routing every read through a `read`/`readSingleValue`/`logStmt` helper that calls `logStatement` under `logEachSelect()` in `CountOptimization`, `KeyCondition`, `ReadInOrderToggle`, `LimitRanking`, `SubqueryMaterialize` (the inline query was the missing one) and `PartitionMirror`, and by embedding both SQL strings in `CountOptimization`'s cross-check message. Copy `ClickHouseStatisticsPartPruningOracle`'s `logStmt` pattern in new oracles; do not make `ComparatorHelper` log for everyone, it is shared with every other DBMS and the reproducers would balloon.

**Nightly plumbing.** `ci/jobs/sqlancer_job.sh` clones this repo's `main` at job start and scrapes `ALL_ORACLES=` out of `.claude/run-sqlancer.sh`, so adding an oracle to that list puts it into the nightly immediately with no ClickHouse-side commit; the job's only escape hatch is its `EXCLUDED_ORACLES` (currently `TextIndexDirectRead`), and a noisy oracle is fixed here, not there. praktika reports for master refs expire within a few days (on 09-01 only the 09-01 report was still fetchable, 08-25/08-28/08-31 returned 403); older runs are triaged from `gh run view --job <id> --log`, which has the analysis summary but no reproducer SQL, so the assertion message is often the only surviving artefact.
