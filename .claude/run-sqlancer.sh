...
# FloatPruning is DELIBERATELY absent from ALL_ORACLES (2026-08-15): it is a positive-control
# detector for ClickHouse #113417/#112036 (NaN rows dropped by float part pruning under a negated
# comparison), which still reproduce on head at DEFAULT settings, so it asserts on essentially every
# iteration -- a 6-minute standalone run produced 326 worker deaths over 175 queries. A constantly
# firing oracle orphans a database per iteration and wedges CH under a squeezed -m cap (that is what
# stalled the 2026-06-14 20h run via TextIndexDirectRead). Run it standalone with
# `--oracles FloatPruning`, and add it back here once those two issues close.
